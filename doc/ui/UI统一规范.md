...
| 配套设计稿 | doc/ui/鸿蒙端页面设计稿.md、doc/ui/管理端页面设计稿.md |
...
| 区域 | 规范 |
| --- | --- |
| 侧边栏 | 宽 220px，浅色底（`--el-fill-color-light`），el-menu 竖向：科室管理 / 医生管理 |
...
| 场景 | 行为 |
| --- | --- |
| 业务错误 | `ElMessage.error(message)`（后端 message 原文） |
| 操作成功 | `ElMessage.success(message)` 后刷新列表/关闭弹窗 |
| 网络异常 | `ElMessage.error('网络连接异常，请检查网络')` |
| HTTP 401 | `ElMessage.warning('登录已失效')` → 跳占位提示页（管理端登录为范围外） |
| 空态 | el-empty（表格 data 为空自带） |
