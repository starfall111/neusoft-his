...
| `db/init.sql` | **数据契约基线（V2.4，定格不再直改）**：六表 DDL + 种子数据；六表齐备 delmark，3 唯一键 + 8 CHECK 约束；后续 DDL 一律走 `db/migrations/` 增量脚本 |
| `db/migrations/` | **增量迁移脚本（V2.4 起）**：表/字段的加入与删除等 DDL 变更的独立 SQL 脚本（命名 `V{版本}__{描述}.sql`，含回滚注释），约定见其 README |
