...
### 规则一 · 特性分支先行（每日开工第一步，仅限代码工作区）

在**代码工作区**（规则四「工作环境登记表」登记的工程仓库，如后端 group-fourteen、vue-admin、鸿蒙工程）开始当日工作（D01–D10）前，必须先在该代码仓库内创建并切换到当日特性分支，禁止直接在 master 上开发：

- 分支基线为 **master**：每日特性分支一律从 master 拉出，完成后合并回 master；不再使用 dev 分支；
- **开工先拉最新代码**：创建当日特性分支前，必须先在 master 上执行拉取（`git pull`）更新到远端最新，确保基线不落后；若当日特性分支已存在（中途续工），切换到该分支后同样先拉取远端最新再继续编码；
- 分支命名：`feature/{成员标识}-D{天数}-{主题简拼}`，例：`feature/a-D03-registration`、`feature/c-D02-admin-dept`；
- 当日收工：推送特性分支并向组长 A 发起合并到 master 的请求；次日开工再从 master 拉当日新分支；
- AI 助手在代码工作区检测到当前处于 master 分支时，必须先提醒：「⚠️ 当前在 master 分支。按规则我先创建今日特性分支 feature/xxx，再开始工作。」
- **本文档仓库（neusoft-his）不适用本条**：计划、日报、exception 记录、契约文档等文档改动直接在文档仓库当前分支提交即可，无需拉特性分支，AI 也不得在文档仓库内提示切换分支。
...
### 规则四 · 编码工作环境确认（登记后免问；未登记禁止编码）

每次收到用户指令时：

1. 先查下方「工作环境登记表」：对应成员/工程的编码工作目录**已登记 → 直接使用，不再询问**；
2. **未登记 → 必须先向用户确认编码工作环境**（该成员代码工程在本机的完整路径），得到答案后，**登记前必须先在该目录执行 `git pull` 拉取远端最新代码并确认成功**（顺带验证该路径存在且为有效代码仓库；拉取失败按规则五记录 exception 并请用户处理，处理完成前不得登记、不得编码），验证通过后立即写入下表（含确认时间），此后不再重复询问；
3. **登记前禁止任何编码行为**：对应工程未登记前，AI 不得创建或修改任何代码文件，只能做需求梳理、方案讲解、Plan 编写、文档整理等工作；
4. **禁止将代码写入文档仓库**：本文档仓库（neusoft-his）只存放文档与过程材料（计划、契约、日报、issue 记录、UI 设计稿等），任何情况下不得在其中创建代码工程或代码文件。

#### 工作环境登记表（已登记则不再询问）

| 对象 | 确认时间 | 编码工作目录（本机完整路径） | 备注 |
| --- | --- | --- | --- |
| 后端仓库（A 使用） | 2026-09-11 20:28 | E:\aaaaa\group-fourteen | 已验证 git pull 成功（5472b71→7c8a96c）；Maven 工程在子目录 neusoft-his/ |
| 后端仓库（B 使用） |  | | |
| vue-admin 管理端（C 使用） | | | |
...
- **目录**：`exception-day{NN}/`（NN=两位天数），**固定位于本文档仓库（neusoft-his）的成员目录下**（如 `member-a/exception-day03/`），当日首次遇到错误时创建；**禁止放入代码工作区仓库内**——即使错误发生在编码时，记录也统一回收到文档仓库；
- **文件**：每个问题一个 `issue-{序号}-{关键词}.md`，必含：时间、错误信息原文（完整堆栈/响应）、发生场景、原因分析、解决方案、验证结果、是否涉契约（涉契约须同时走铁律对齐）；
- **收工检查**：当日 DoD 暴露过的问题都能在文档仓库的 exception 目录找到对应记录；目录随本文档仓库提交，作为过程性考核与答辩「踩坑与解决」素材。
...
1. **第一步 · 确认当前成员是谁（必做；身份不明禁止开工）**：先查规则十「用户身份登记表」，已登记 → 直接使用登记结果；未登记时优先从当前工作上下文判断（正在操作哪个 `member-x` 目录、该目录 AGENTS.md 角色卡）；**仍无法唯一确定时，必须先询问用户「当前是哪位成员（A/B/C/D）？」**，确认后按规则十写入登记表再继续；
...
### 规则十 · 用户身份确认（每次需求开工第一步，登记后免问）

每次用户提出需求时，AI 必须先确认当前用户的成员身份，再开始执行：

1. 先查下方「用户身份登记表」：当前用户身份**已登记 → 直接使用登记结果，不再询问**；
2. **未登记 → 必须先询问用户「当前是哪位成员（A/B/C/D）？」**，得到答案后立即写入下表（含确认时间），此后不再重复询问；
3. 身份确认后才继续执行该需求；开工前置共两步：**规则十（身份确认）+ 规则十一（拉取文档仓库最新 master 更新今日任务）**；后续规则的联动顺序为：涉及编码 → 再过规则四（工作环境确认）→ 规则二（Plan 审核）；生成日报 → 按规则九以已确认身份收集本成员数据；
4. **身份未确认前，禁止以任何成员名义产出归档材料**（日计划、日报、issue 记录、契约变更记录等），禁止跨成员代写。

#### 用户身份登记表（已登记则不再询问）

| 成员 | 角色 | 确认时间 | 备注 |
| --- | --- | --- | --- |
| 成员 A | 后端 | 2026-09-11 20:28 | D03 开工自报，重新登记 |
| 成员 B | 后端 | | |
| 成员 C | vue-admin 管理端 | | |
| 成员 D | HarmonyOS 患者端 | | |

### 规则十一 · 文档仓库先行拉取（每次需求开工同步动作，更新今日任务）

每次用户提出需求后，AI 必须先在**本文档仓库（neusoft-his）**执行拉取（`git pull` 更新 master 分支），获取最新的当日日计划、契约文档（db/）、exception 记录与公共规范，再基于最新版本继续工作：

- 执行顺序：与规则十并列为「需求开工两步前置」——**先确认身份（规则十，登记后免问），再拉取文档基线（本规则）**，之后才进入需求本身的流程（涉及编码 → 规则四 → 规则二）；
- 拉取失败或出现冲突时：暂停后续任何写入动作，当场按规则五记录到当前成员的 `exception-day{NN}/`，并请用户处理，处理完再继续；
- 拉取后若发现当日日计划被更新（如任务内容、DoD 变化），以拉取到的最新版本为准，并向用户口头复述当日任务要点后再开工。
...
### 铁律一 · 数据库 DDL 变更（表/字段增删改、类型、索引、约束、种子数据调整）
提醒话术：「⚠️ 本次修改涉及数据库 DDL。请先与后端 A、B 及依赖该表的前端 C、D 对齐（站会或群内）；由 B 统一编写**增量迁移脚本** `db/migrations/V{版本}__{描述}.sql` 并更新《项目拆解设计说明书》第 6 章，同步 db/mock-data.md；各成员确认后在自己的当日计划文档『契约变更记录』表追加记录并调整后续计划。」

> **迁移脚本规则（组长 2026-09-10 定）**：自 V2.4 起，一切数据库结构变更（表/字段的加入与删除、类型、索引、约束及随之的种子调整）一律通过 `db/migrations/` 下的**独立 SQL 脚本**交付，**禁止再直接修改 db/init.sql**；init.sql 定格为 V2.4 全量基线，全新环境按「init.sql → migrations 按版本号升序逐个执行」初始化；已提交脚本禁止修改与删除（改错用新脚本修正）；命名、模板与已应用清单见 `db/migrations/README.md`。
...
- Git（仅代码工作区仓库）：**master 为唯一基线与日常集成分支**（禁止直接开发），feature/* 每日特性分支一律从 master 拉出、完成后合并回 master（命名见第 3 节规则一），不使用 dev 分支；本文档仓库（neusoft-his）文档改动直接在当前分支提交，不拉特性分支。commit 前缀 feat:/fix:/optimize:
