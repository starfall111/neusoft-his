# 成员A（组长 · 后端 · 架构/挂号域/中间件） · 第 8 天：扩展-MQ与异常兜底
...
1. RocketMQMessageSender（topic：his_registration_success；消息体：orderNo/userId/memberName/workDate/timeSegment）
2. MQ 不可用时降级 LoggingMessageSender，保证可演示
3. 全局异常兜底复查：NPE、参数缺失、Redis 断连时的降级路径
