# AGENTS.md · 成员C（前端 · Vue3 管理端） 工作规则
...
- **角色**：成员C（前端 · Vue3 管理端）
- **职责**：负责 vue-admin 管理端（S11/S12 界面）与联调，第 5–6 天硬性支援鸿蒙端静态页面，答辩截图素材库维护。
- **关联故事**：S11 / S12（Web 管理端）+ 支援 S8/S9 静态页
...
## 6. 契约变更三条铁律（AI 助手必须主动提醒，未对齐前不得动手改契约）
