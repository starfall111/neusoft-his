...
**GET /departments（S4）、GET /admin/departments（S11） · DepartmentRespDto（响应；V2.3 父子两级平铺）**
...
**GET /quotas?deptId=&date=（V1.9 新增） · QuotaRespDto（响应，List）**：`scheduleId, doctorId, doctorName, title, skill, timeSegment, leftQuota, registerFee`（后端排序先上午后下午、时段内职称主任→副主任→主治）

**GET /quotas/days?deptId=（V1.9 新增） · QuotaDayRespDto（响应，List）**：`date`（当天+1~+7）+ `dayStatus`（可约/已满/休）
...
| parentId | Long | 是 | @NotNull；父分类节点 id（V2.3 改传 id 不再传分类文本） |
