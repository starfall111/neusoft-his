...
1. S6 GET /schedules?doctorId=：近 7 天（当天+1~+7），无排班返回空数组
2. S7 POST /members（上限 3 人→2001；Hutool IdcardUtil→2002）、GET /members
3. S3 POST /auth/logout
4. S11 /admin/departments 四接口（Flex paginate → PageDTO，字段按 mock-data.md）
5. S12 /admin/doctors 四接口（deptId 不存在→4001；删除医生=逻辑删除 delmark=1 且同事务将其未来排班逻辑删除 delmark=1，见 6.5-②）
6. 10 接口全部 Apifox 实测并与 db/mock-data.md 逐字段 diff；填第 10 节对照表（规则八）
