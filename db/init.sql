...
--   ⚠️ 自 V2.4 起 init.sql 定格为全量基线、不再直接修改——后续一切表/字段增删改（DDL）
--     一律通过 db/migrations/ 下的独立增量脚本交付（命名 V{版本}__{描述}.sql，含回滚注释），
--     约定见 db/migrations/README.md 与 AGENTS.md 铁律一（组长 2026-09-10 决策）。
-- 执行方式：全新环境 mysql -uroot -p < init.sql 后，按版本号升序执行 db/migrations/*.sql
-- ============================================================
